...
public class Model {
	...
	public Model(Model other) {
		landscape = new boolean [other.getRows()][other.getCols()];
		...
		for(int i =0; i < other.fish.size(); i++) {
			Fish x = new Fish(other.fish.get(i));
			this.fish.add(x);
		}
		this.plants = new ArrayList<Plant>();
		for(int i =0; i < other.plants.size(); i++) {
			Plant x = new Plant(other.plants.get(i));
			this.plants.add(x);
		}
	...
	public void turnFish() {
		for(Fish x: fish) {
			if(x.isAlive()==true) {
				if(fishIsSurroundedByRocks(x)==true) {
					continue;
				}else if(x.getDirection()!= Fish.DOWN
						&& x.getDirection() != Fish.LEFT 
						&&x.getDirection() != Fish.UP 
						&& x.getDirection() != Fish.RIGHT ) {
					throw new IllegalFishDirectionException(x.getDirection());
				}
				while((x.getDirection()== Fish.UP && 
						landscape[x.getRow()-1][x.getCol()]==ROCK) || 
						(x.getDirection() == Fish.DOWN && 
						landscape[x.getRow()+1][x.getCol()]==ROCK) ||
						(x.getDirection() == Fish.LEFT && 
						landscape[x.getRow()][x.getCol()-1]==ROCK) ||
						(x.getDirection() == Fish.RIGHT && 
						landscape[x.getRow()][x.getCol()+1]==ROCK)) {
		...
		for(Fish x: fish) {
			if(x.isAlive()==true) {
				if(fishIsSurroundedByRocks(x)==true) {
					...
					x.move();
					for(Plant y: plants) {
						if(y.isAlive() && (y.getCol() == x.getCol()) && 
								(y.getRow() == x.getRow())) {
							fishEatPlant(x,y);
						}
					}
					for(Fish z: fish) {
						if(x.getRow() == z.getRow() && x.getCol() == z.getCol()
								&& x!=z) {
							x.fight(z);
						}
					}
	...
	public void addPlant(Plant p) {
		int h =0;
		for(Plant x:plants) {
			if(x.getRow() == p.getRow() && x.getCol()== p.getCol()) {
				h++;
			}
		}
	...
	public void addFish(Fish f){
		int h =0;
		for(Fish x:fish) {
			if(x.getRow() == f.getRow() && x.getCol()== f.getCol() && x!=f) {
				h++;
			}
		}
	...
	public ArrayList<Fish> getFish() {
		ArrayList<Fish> Other = new ArrayList<Fish>(fish);
		return Other;
	}
	...
	public ArrayList<Plant> getPlants() {
		ArrayList<Plant> Other = new ArrayList<Plant>(plants);
		return Other;
	}
